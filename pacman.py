...
class PacMan:
    ...
    def draw(self):
        pacman_sprite = SPRITES["pacman"]
        screen.blit(pacman_sprite, (self.x * TILE_SIZE, self.y * TILE_SIZE))
    ...
    def draw_hud(self):
        ...
        screen.blit(
            lives_text,
            (SCREEN_WIDTH - lives_text.get_width() - 10, 10)
        )
        if self.powered_up:
            ...
            screen.blit(
                power_text,
                (SCREEN_WIDTH // 2 - power_text.get_width() // 2, 10)
            )
